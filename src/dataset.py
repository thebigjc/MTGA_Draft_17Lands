...
class Dataset:
    def __init__(self, retrieve_unknown: bool = False):
        self._dataset = None
        self._local_data = {}  # This will eventually contain only data for the active set
        self._retrieve_unknown = retrieve_unknown
        
    def _load_local_data(self, active_set: str) -> None:
        """
        Attempts to load the local card data file generated by FileExtractor, which is structured
        as a dictionary keyed by the set code. Only load the data for the active set using ijson
        to avoid loading the full file into memory.
        """
        local_file = TEMP_CARD_DATA_FILE
        logger.info(f"Loading local card data from {local_file}")
        if os.path.exists(local_file):
            try:
                with open(local_file, "r", encoding="utf-8", errors="replace") as f:
                    # Iterate over top-level key/value pairs
                    found = False
                    for set_key, cards in ijson.kvitems(f, ''):
                        if set_key == active_set:
                            self._local_data = cards
                            found = True
                            break
                    if not found:
                        self._local_data = {}
            except Exception as error:
                print("Error loading local card data: ", error)
                self._local_data = {}
        else:
            self._local_data = {}
    ...
    def open_file(self, file_location: str) -> None:
        ...
        active_set = json_data["meta"]["set_code"]
        self._load_local_data(active_set)
